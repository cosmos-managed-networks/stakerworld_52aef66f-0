...
HTTPS_GIT := github.com/stakerworld/stakerworld.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=stakerworld \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=stakerworldd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/stakerworldd ./cmd/stakerworldd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/stakerworldd.exe ./cmd/stakerworldd
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/stakerworldd
...
draw-deps:
	...
	@goviz -i ./cmd/stakerworldd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c stakerworld --local -f chains.yaml
endif
...
set-testnet-configs:
	stakerworldd config set client chain-id chainid-1
	stakerworldd config set client keyring-backend test
	stakerworldd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | stakerworldd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | stakerworldd keys add acc1 --recover`
