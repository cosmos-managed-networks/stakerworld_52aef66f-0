...
# then log output of file /code/bin/stakerworldd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/stakerworldd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/stakerworldd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/stakerworldd /usr/bin/stakerworldd
...
CMD ["/usr/bin/stakerworldd", "version"]
